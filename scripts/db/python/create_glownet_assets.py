...
def create_customer(event_id, cNumber):
    ...
    print(f"  Creating customer {cNumber}: {payload['customer']['email']}...", end="", flush=True)
    try:
        FAKE.unique.clear()
...
if __name__ == "__main__":
    ...
    if ask_yes_no(f"Create customers for event '{target_event_id}'?"):
        ...
        for i in range(customers_to_create):
            customer_info = create_customer(target_event_id, custNumber)
